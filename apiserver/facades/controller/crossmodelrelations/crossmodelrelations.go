...
type egressAddressWatcherFunc func(facade.Resources, firewall.State, firewall.ModelConfigService, firewall.ApplicationService, params.Entities) (params.StringsWatchResults, error)
...
func (api *CrossModelRelationsAPIv3) WatchEgressAddressesForRelations(ctx context.Context, remoteRelationArgs params.RemoteEntityArgs) (params.StringsWatchResults, error) {
	...
	watchResults, err := api.egressAddressWatcher(api.resources, api.fw, api.modelConfigService, api.applicationService, relations)
	if err != nil {
		return results, err
	}
	index := 0
