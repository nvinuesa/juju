...
func (s *MachineFSMSuite) TestAllowedTransitions(c *tc.C) {
	testCases := []struct {
		...
		// Reserved states
		{provisionertask.StateAllocated, provisionertask.StateProvisioning, "Allocated to Provisioning"},
		{provisionertask.StateAllocated, provisionertask.StateStopping, "Allocated to Stopping"},
		{provisionertask.StateProvisioning, provisionertask.StateRunning, "Provisioning to Running"},
		{provisionertask.StateProvisioning, provisionertask.StateStopping, "Provisioning to Stopping"},
	}
...
func (s *MachineFSMSuite) TestIllegalTransitions(c *tc.C) {
	testCases := []struct {
		...
		// From Dead - terminal state, only Dead is allowed (tested in idempotent)
		...
		// From Failed - terminal state, only Failed is allowed (tested in idempotent)
...
func (s *MachineFSMSuite) TestTerminalStates(c *tc.C) {
	...
	c.Assert(provisionertask.IsTerminal(provisionertask.StateAllocated), tc.IsFalse)
	c.Assert(provisionertask.IsTerminal(provisionertask.StateProvisioning), tc.IsFalse)
}
...
func (s *MachineFSMSuite) TestCompleteLifecycle(c *tc.C) {
	// Test a complete lifecycle: Pending -> Starting -> Running -> Stopping -> Dead
...
func (s *MachineFSMSuite) TestCancellationLifecycle(c *tc.C) {
	// Test cancellation during start: Pending -> Starting -> CancellingStart -> Stopping -> Dead
