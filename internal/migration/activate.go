...
type ActivateModelArgs struct {
	...
	// SourceControllerUUID is the UUID of the source controller (used for
	// CMR offerer reconciliation in commit 2).
	SourceControllerUUID string

	// SourceControllerAlias is the human-readable alias of the source
	// controller (used for CMR offerer reconciliation in commit 2).
	SourceControllerAlias string

	// SourceCACert is the CA certificate of the source controller (used for
	// CMR offerer reconciliation in commit 2).
	SourceCACert string

	// SourceAPIAddrs are the API addresses of the source controller (used for
	// CMR offerer reconciliation in commit 2).
	SourceAPIAddrs []string

	// CrossModelUUIDs are the model UUIDs that have cross-model relations to
	// the source controller after migration (used for CMR offerer
	// reconciliation in commit 2).
	CrossModelUUIDs []string
}

// ActivateModel finalises the activation of a model imported via the v8 path,
// implementing the §4.8 phase machine: importing → activating → claim deleted.
// It is idempotent: retrying after a crash at any step resumes safely.
//
...
//
// The §4.8 reconcile slot (CMR offerer-controller walk) is filled in commit 2.
...
//     source controller (WS9 test 14).
//
//   - Third-party offerers: applications whose offering model is hosted on a
//     controller other than the source, recorded in
//     model_migration_import_external_controller_model during import (WS9
//     test 14a). Only present on the v8 path (hasClaim).
//
