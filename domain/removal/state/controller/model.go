...
// database is gone before releasing the model UUID - never runs. Destroying a
// migrating model is instead routed through the migration abort path, which
// owns that protocol. The check is inside the transaction that begins removal,
// so a claim appearing concurrently cannot slip past it.
func (st *State) EnsureModelNotAlive(ctx context.Context, modelUUID string, force bool) error {
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		if hasClaim, phase, err := st.migratingImportPhase(ctx, tx, modelUUID); err != nil {
			return errors.Errorf("checking migration import claim: %w", err)
		} else if hasClaim {
			return errors.Errorf(
				"model %q has a migration import claim in the %q phase: %w",
				modelUUID, phase, removalerrors.MigrationImportActive)
		}
