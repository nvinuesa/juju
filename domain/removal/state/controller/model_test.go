...
//
// The service routes such a model to the migration abort path, but that is a
// check-then-act decision: a claim can appear between the check and this call.
// The refusal lives inside the transaction that begins removal, so the race
// fails closed - the model stays alive and the import keeps its claim, rather
// than being torn down mid-write with its cleanup protocol skipped.
...
VALUES ('claim-uuid', ?, 'source-migration-uuid')`, modelUUID)
	...
	// Force must not be an escape hatch here: the objection is not about the
	// model's contents but about who owns its teardown.
