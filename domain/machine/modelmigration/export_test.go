...
func (s *exportSuite) TestFailRetrieveAllMachinesForExport(c *gc.C) {
	defer s.setupMocks(c).Finish()

	dst := description.NewModel(description.ModelArgs{})

	s.service.EXPECT().AllMachineUUIDs(gomock.Any()).
		Return([]string{}, errors.New("boom"))

	op := s.newExportOperation(c)
	err := op.Execute(context.Background(), dst)
	c.Assert(err, gc.ErrorMatches, "retrieving all machines for export: boom")
}
...
func (s *exportSuite) TestFailGetInstanceIDForExport(c *gc.C) {
	...
	op := s.newExportOperation(c)
	err := op.Execute(context.Background(), dst)
	c.Assert(err, gc.ErrorMatches, "retrieving instance ID for machine \"deadbeef-0bad-400d-8000-4b1d0d06f00d\": boom")
}
...
func (s *exportSuite) TestFailGetHardwareCharacteristicsForExport(c *gc.C) {
	...
	op := s.newExportOperation(c)
	err := op.Execute(context.Background(), dst)
	c.Assert(err, gc.ErrorMatches, "retrieving hardware characteristics for machine \"deadbeef-0bad-400d-8000-4b1d0d06f00d\": boom")
}
...
func (s *exportSuite) TestExport(c *gc.C) {
	...
	c.Check(actualMachines[0].Id(), gc.Equals, machineUUIDs[0])
