...
type migrationTargetAuth struct {
	...
	TargetPassword         string `db:"target_password"`
...
// migrationPhaseEntry maps a model_migration_export_phase history row. model_uuid
// is denormalised from the parent export row so the changestream trigger emits
// it (the phase namespace is model-scoped).
...
// currentPhase is the projection of the export's denormalised current phase plus
// the model uuid needed to stamp model-scoped phase-history rows.
