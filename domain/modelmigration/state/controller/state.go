...
// [modelmigration.ErrMigrationAlreadyActive].
func (s *State) InsertExport(ctx context.Context, spec modelmigration.MigrationSpec) error {
	...
	auth := migrationTargetAuth{
		...
		TargetPassword:         spec.Target.Password,
		...
		if err := tx.Query(ctx, insertExportStmt, export).Run(); err != nil {
			if database.IsErrConstraintUnique(err) {
				return errors.Errorf(
					...
					spec.ModelUUID, modelmigration.ErrMigrationAlreadyActive,
				)
			}
...
// returns [modelmigration.ErrExternalControllerConflict] if a controller with
...
`, ctrlUUID, addressValue{})
	...
	if existing.CACert != info.CACert {
		return errors.Errorf(
			...
			info.UUID, modelmigration.ErrExternalControllerConflict,
		)
	}
...
// export exists [modelmigration.ErrMigrationNotFound] is returned.
...
`, mUUID, migrationExport{})
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("no active migration for model %q: %w", modelUUID, modelmigration.ErrMigrationNotFound)
		} else if err != nil {
...
// [modelmigration.ErrPhaseTransitionInvalid].
...
SELECT &currentPhase.current_phase_id, &currentPhase.model_uuid
FROM   model_migration_export
WHERE  uuid = $entityUUID.uuid
AND    end_time IS NULL
...
`, phaseEntry)
	...
	return db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("no active migration %q: %w", migrationUUID, modelmigration.ErrMigrationNotFound)
		} else if err != nil {
		...
		if !curPhase.CanTransitionTo(newPhase) {
			return errors.Errorf(
				...
				migrationUUID, curPhase, newPhase, modelmigration.ErrPhaseTransitionInvalid,
			)
		}
		...
		if affected != 1 {
			return errors.Errorf(
				...
				migrationUUID, modelmigration.ErrPhaseTransitionInvalid,
			)
		}

		// Stamp the model uuid read from the export row so the phase-history
		// row carries the model-scoped changestream key.
		phaseEntry.ModelUUID = cur.ModelUUID
		if err := tx.Query(ctx, insertPhaseStmt, phaseEntry).Run(); err != nil {
...
`, end)
	...
	migUUID := entityUUID{UUID: migrationUUID}
	selectModelStmt, err := s.Prepare(`
SELECT &currentPhase.model_uuid
FROM   model_migration_export
WHERE  uuid = $entityUUID.uuid
`, migUUID, currentPhase{})
	if err != nil {
...
`, phaseEntry)
	...
	return db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		// Read the model uuid before ending so the terminal phase-history row
		// carries the model-scoped changestream key.
		var cur currentPhase
		if err := tx.Query(ctx, selectModelStmt, migUUID).Get(&cur); err != nil {
			if errors.Is(err, sqlair.ErrNoRows) {
				return errors.Errorf("no migration %q: %w", migrationUUID, modelmigration.ErrMigrationNotFound)
			}
			return errors.Errorf("reading model uuid for migration %q: %w", migrationUUID, err)
		}
...
// export migration starts or ends (keyed by model_uuid). It deliberately does
// not fire on phase transitions; use NamespaceForWatchPhase for those.
...
// export migration phase transition (keyed by model_uuid).
...
// when a minion sync report changes (keyed by migration_uuid).
