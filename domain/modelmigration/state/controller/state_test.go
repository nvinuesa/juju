...
func (s *stateSuite) TestInsertExport(c *tc.C) {
	...
	err = db.QueryRowContext(c.Context(),
		"SELECT target_user, target_password FROM model_migration_export_target_auth WHERE migration_uuid = ?",
		spec.MigrationUUID).Scan(&user, &password)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(user, tc.Equals, "admin")
	c.Check(password, tc.Equals, "super-secret")

	// Phase history seeded with QUIESCE, carrying the denormalised model_uuid
	// used as the phase changestream key.
	var phaseModelUUID string
	err = db.QueryRowContext(c.Context(),
		"SELECT model_uuid FROM model_migration_export_phase WHERE migration_uuid = ? AND phase_id = 1",
		spec.MigrationUUID).Scan(&phaseModelUUID)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(phaseModelUUID, tc.Equals, s.modelUUID.String())
...
func (s *stateSuite) TestInsertExportAlreadyActive(c *tc.C) {
	...
	err = st.InsertExport(c.Context(), s.newMigrationSpec())
	c.Assert(err, tc.ErrorIs, modelmigration.ErrMigrationAlreadyActive)
}
...
func (s *stateSuite) TestEnsureExternalControllerCACertConflict(c *tc.C) {
	...
	err = st.EnsureExternalControllerMatchesOrInsert(c.Context(), info)
	c.Assert(err, tc.ErrorIs, modelmigration.ErrExternalControllerConflict)
}
...
func (s *stateSuite) TestGetActiveExport(c *tc.C) {
	...
	c.Check(mig.Target.Password, tc.Equals, "super-secret")
...
func (s *stateSuite) TestSetPhaseValidTransition(c *tc.C) {
	...
	var historyModelUUID string
	err = db.QueryRowContext(c.Context(),
		"SELECT model_uuid FROM model_migration_export_phase WHERE migration_uuid = ? AND phase_id = 2",
		spec.MigrationUUID).Scan(&historyModelUUID)
	c.Assert(err, tc.ErrorIsNil)
	c.Check(historyModelUUID, tc.Equals, s.modelUUID.String())
}

// TestSetPhaseInvalidTransition asserts an invalid transition is rejected with
// [modelmigration.ErrPhaseTransitionInvalid] and leaves the phase unchanged.
func (s *stateSuite) TestSetPhaseInvalidTransition(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIs, modelmigration.ErrPhaseTransitionInvalid)
